/*
  # InsightsLM Complete Database Schema

  1. New Tables
    - `profiles` - User profile information
    - `notebooks` - User's notebooks for organizing content
    - `sources` - Content sources (PDFs, websites, videos, audio, text)
  ...
  2. Custom Types
    - `source_type` - ENUM for different source types
  ...
  5. Features
    ...
    - Storage buckets for files and audio with access control

  Important Notes:
    - The profiles table references auth.users and deletes on cascade
    - All timestamps are UTC-based
    - RLS policies ensure users can only access their own data
    - Storage policies use folder structure to organize by notebook
...
DO $$ BEGIN
  EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE public.notebooks';
...
DO $$ BEGIN
  EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE public.n8n_chat_histories';
EXCEPTION WHEN OTHERS THEN NULL;
END $$;
